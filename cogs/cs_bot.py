...
class Cs_Cog(commands.Cog):
	...
	async def server_status(self):
		...
		try:
			...
			print('server_info')
			print(server_info)
			if(server_info.current_map != self.cs16_map):
				self.cs16_map = server_info.current_map
				#print('server_info.players')
				#print(server_info.players)
				PlayersOnlineStr = '\n'.join(server_info.players) #playersOnline.join('\n')
				#print(PlayersOnlineStr)
				...
				async with aiohttp.ClientSession() as session: # creates session
					async with session.get(config.SERVER_STATUS_URL) as resp: # gets image from url
						...
						with io.BytesIO(img) as file: # converts to file-like object
							#channel.send('connect 46.174.52.22:27212')
							ch = self.bot.get_channel(Cs_Cog.cs_channel)
							c_time = await self.command_time(startCmdTime);
							await ch.send(f'\n{msg}\n```connect 46.174.52.22:27212```', file=discord.File(file, "server_status.png"))
							await self.lem_log(startCmdTime, self.bot.user ,'/server status cycle', c_time, ch.guild.name)
		except Exception as e:
			ch = self.bot.get_channel(Cs_Cog.cs_channel)
			await ch.send(f'\n```Сервер недоступен```')
			print(e)
			await self.lem_log_stack_trace(startCmdTime, self.bot.user ,'server status cycle', 0, ch.guild.name, traceback.format_exc())
	...
	#def timer_init(self):
	#	timer_status = threading.Timer(5,self.test)
			...
			async with aiohttp.ClientSession() as session: # creates session
				async with session.get(config.SERVER_STATUS_URL) as resp: # gets image from url
					...
					with io.BytesIO(img) as file: # converts to file-like object
						...
						await interaction.followup.send(f'\n{msg}\n Подключиться: ```connect 46.174.52.22:27212```', file=discord.File(file, "server_status.png"))#, embed = emb)
	...
	async def nick_cs(self, interaction: discord.Interaction, nick: str):
		await interaction.response.send_message(f'``` '+ database.setPlayer(interaction.user.name + '#' + interaction.user.discriminator ,nick) + '```')
